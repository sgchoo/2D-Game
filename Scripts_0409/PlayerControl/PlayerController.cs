...
//Summary : 플레이어 기본 이동 및 점프, 더블 점프, 벽점프,
//          대쉬, 마우스(0번)클릭 시 월드 좌표로 짧게 이동
...
public class PlayerController : MonoBehaviour
{
    ...
    private void WallJump()
    {
        ...
        {
            isWallJump = true;
            rigid.velocity = new Vector2(0,0).normalized;
        }
    ...
    void AttackToMouseDirection()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            if (Mathf.Abs(mouseDirection.y) > Mathf.Abs(mouseDirection.x))
            {
                ...
                {
                    simpleShootingDirection = Vector2.up * 10f;
                }
                else
                {
                    simpleShootingDirection = Vector2.down * 10f;
                }
            }
            ...
            else
            {
                ...
                {
                    simpleShootingDirection = Vector2.right * attackSpeed;
                }
                else
                {
                    simpleShootingDirection = Vector2.left * attackSpeed;
                }
            }

            rigid.velocity = new Vector2(simpleShootingDirection.x, simpleShootingDirection.y);

            if (simpleShootingDirection.x > 0)
            {
                sprite.flipX = false;
            }

            else if (simpleShootingDirection.x < 0)
            {
                sprite.flipX = true;
            }
        }
